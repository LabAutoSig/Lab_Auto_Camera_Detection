...
def findArucoMarkers(img,img_folder_path, id_list , totalMarkers=1000, draw = False): #image path is used
    ...
    print(f"Detected bounding bboxs: {bboxs} and IDs:{ids}")
    ...
    if len(bboxs) <= 1: #Print if no markers are detected
        ...
        bboxs = None
        ids = None
        image_enhanced = run_image_enhancer(img_folder_path)
        resized_enhanced = cv2.resize(image_enhanced, (new_width, new_height))
        image_return = resized_enhanced
        cv2.imshow("Enhanced image", resized_enhanced)
        gray_enhanced = cv2.cvtColor(resized_enhanced, cv2.COLOR_BGR2GRAY)#transform image into grayscale
        bboxs, ids, _ = aruco.detectMarkers(gray_enhanced, arucoDict, parameters = arucoParam)
        if draw:
...
def run_image_enhancer(img_folder_path):
    ...
    autoit_exe_path = r'C:\Users\wienbruch\autoit-v3\install\AutoIt3.exe'

    # Path to your AutoIt script
    autoit_script_path = r'C:\Users\wienbruch\venv311\Detection_system_06_02_24\Image_Enhancer\Cutout_Edge_WD_3_RW.au3'
    ...
    img_enhanced = cv2.imread("C:\\Users\\wienbruch\\venv311\\Detection_system_06_02_24\\Enhanced_imgs\\enhanced_img.png")
...
def processImage(bboxs, ids,img): # use the found aruco marker coordinates and ids
    ...
    if lu_id1[0] < lu_id2[0]: #if left upper x-coordinate value of id 2 is greater than of id1
        #left marker == id1 --> ru/rl corner
        #right marker == id2 --> lu/ll corner
        print(f"left marker ID{id1}")
        if lu_id1[1] < lu_id2[1]:#left upper y-coordinate of id2 greater than of id1
            ...
            ll_x = int(ll_id2[0]) 
            ll_y = int(ll_id2[1])
            img_res = img[ru_x:ll_x, ru_y:ll_y]#crop image
            #print(img_res)
            #cv2.imshow("Cropped", img_res)#Show cropped image
        elif lu_id1[1] > lu_id2[1]:#left upper y-coordinate of id1 greater than of id2
            #Id1 = lower marker --> rl
            print(f"left lower marker ID{id1}")
            rl_x = int(rl_id1[0]) 
            rl_y = int(rl_id1[1])
            lu_x = int(lu_id2[0]) 
            lu_y = int(lu_id2[1])
            print(f"rl marker: {rl_x} , {rl_y} \n lu marker: {lu_x} , {lu_y}")
            img_res = img[rl_x:lu_x, lu_y:rl_y] #crop image
            #print(img_res)
            cv2.imshow("Cropped", img_res)#Show cropped image
            
        
    elif lu_id1[0] > lu_id2[0]: # else if left upper x-coordinate value of id 1 is greater than of id2
        #right marker == id1 --> lu/ll corner
        #left marker == id2 --> ru/rl corner
        print(f"right marker ID{id1}")
        if lu_id1[1] < lu_id2[1]:#left upper y-coordinate of id2 greater than of id1
            #Id1 = upper marker --> lu
            #print(f"right upper marker ID {ID1}")
            rl_x = int(rl_id2[0]) 
            rl_y = int(rl_id2[1])
            lu_x = int(lu_id1[0]) 
            lu_y = int(lu_id1[1])
            img_res = img[lu_y:rl_y, rl_x:lu_x]#crop image
            #print(img_res)
            #cv2.imshow("Cropped", img_res)#Show cropped image
        elif lu_id1[1] > lu_id2[1]:#left upper y-coordinate of id1 greater than of id2
            #Id1 = lower marker --> ll
...
"""def check_row(df_new):
    ...
    dif = df_new["y_bottom"].diff() #calculate difference between bottom x coordinates
    print(f"Difference y_bottom {dif}")
    height = df_new["y_bottom"]
    if len(df_new) > 1: 
        for i in range(len(df_new["y_bottom"])): # if i is in range of the length of y_bottom
            
            if  dif[i] < -0.15 or dif[i] > 0.15: #if the difference between the bottom y coordinates is... #changed from 0.1 to 0.15 check for other displays
                ...
                if height1 > height2: #if y_bottom pixel coordinate  at i-1 is bigger
                    ...
                    print(f"{i}-1 is lower row")
                    dataframe1 = df_new.drop(labels = i-1, axis = 0)
                    print(f"Dataframe1 without lower row {dataframe1}")
                    new_row = df_new.iloc[i-1]
                    ...
                    dataframe1 = df_new.drop(labels = i, axis = 0)
                    print(f"Dataframe1 without lower row {dataframe1}")
    ...
    if len(dataframe1)> 1:
        ...
        for j in range(len(dataframe1["x_bottom"])): #as long as j is in range of length x_bottom
            ...
            if dif[j] > 0.2: #if the difference is greater than 0.2
                ...
                """val1 = float(''.join(map(str, df1[0:j]))) #join Name values 0 till j as float
                print("First value: ",val1)
                val2 = float(''.join(map(str, df1[j:]))) #join Name values j and the rest as float
                    
                print("Second value: ",val2)
                values = [] #clear values array
                values.append(val1) #append value1
                values.append(val2) #append value2"""
...
def data_processing(data_folder_path):

    autoit_SciTe_path = r'C:\Users\wienbruch\autoit-v3\install\SciTe\SciTe.exe'
    ...
    # Path to your AutoIt script
    autoit_script_path = r'C:\Users\wienbruch\venv311\Detection_system_06_02_24\Data_processing\Projekt_V1_11_RW_english_07_02.au3'
...
img_path = r"C:\Users\wienbruch\venv311\Detection_system_06_02_24\Input_img\Variation_lighting\rawimg102.jpg"
img = cv2.imread(img_path)
#cv2.imshow("img",img)
#Define the model path 
yolo_path = "C:\\Users\\wienbruch\\venv311\\Detection_system_06_02_24\\Trained_model\\Model_2_3_2_best.pt"
#Define where the images should be saved
save_path = 'C:\\Users\\wienbruch\\venv311\\Detection_system_06_02_24\\Saved_Images\\'
cropped_path = r'C:\Users\wienbruch\venv311\Detection_system_06_02_24\Processed_imgs\cropped_img.jpg'
output_path = r'C:\Users\wienbruch\venv311\Detection_system_06_02_24\output.csv'
enhanced_path = "C:\\Users\\wienbruch\\venv311\\Detection_system_06_02_24\\Enhanced_imgs\\enhanced_img.png"
