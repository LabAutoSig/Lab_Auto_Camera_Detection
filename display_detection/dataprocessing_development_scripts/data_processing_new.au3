...
;MsgBox(0,"Scite",$scitePath)
;MsgBox(0,"Filename", $file)

;Open SciTe and Output-File ----------------------------------------------------------------------------------------------------------------------
Sleep (100)
Global $csvDatei = $CmdLine[2]
...
Send("^s")
Sleep(100)
$active = WinActivate("output.csv - SciTE-Lite")
;Count output file lines -----------------------------------------------------------------------------------------------------------
Global $anzahlzeilen = _FileCountLines ($active)
Sleep (100)
MsgBox(0,"Zeilen", $anzahlzeilen)
...
Global $readline4 = FileReadLine ($csvDatei, 4) ;ClipPut = Save text in clipboard
Sleep (100)
MsgBox(0,"Line4",$readline4)
Sleep(100)
;Global $ID1 = ClipGet () ;ClipGet = call text from clipboard
;Sleep(100)
;Global $einfuegenID1 = $readline4
Sleep(100)
;Read marker ID from line 5 for Excel file identification -------------------------------------------------------------------------
Global $readline5 = FileReadLine ($csvDatei, 5)
MsgBox(0,"Line5",$readline5)
Sleep (100)
;Global $ID2 = ClipGet ()
Global $einfuegenID2 = $readline5
...
Sleep (100)
MsgBox(0,"ID2",$readline5)
Local $result = Check_Device($einfuegenID1, $einfuegenID2)
$file = $result[0]
MsgBox(0,"File",$file)
$title = $result[1]
$axis_title = $result[2]
